...
public class Greedy {
    ...
    public static LinkedList<Activity> activitySelection(LinkedList<Activity> activities) {
        // TASK 1.B.a
        LinkedList<Activity> SelectedAct = new LinkedList<>();
        Activity lastinput = activities.getFirst();
        SelectedAct.add(lastinput);

        for (Activity pointer : activities) {
            if (!pointer.overlap(lastinput)) {
                SelectedAct.add(pointer);
                lastinput = pointer;
            }
